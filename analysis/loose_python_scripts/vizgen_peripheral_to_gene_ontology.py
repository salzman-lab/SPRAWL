...
viz_df = pd.read_csv('../outputs/gene_cell/Vizgen_Brainmap_peripheral.csv')
...
viz_df = viz_df.groupby(['sample_id','gene','annotation']).filter(lambda g: len(g) >= 20)
...
viz_agg_df.to_csv('../outputs/gene_ontology/Vizgen_Brainmap_peripheral.csv',index=False)
