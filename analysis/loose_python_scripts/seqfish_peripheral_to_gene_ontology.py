...
seq_df = pd.read_csv('../outputs/gene_cell/SeqFish_peripheral.csv')
...
seq_df = seq_df.groupby(['sample_id','gene','annotation']).filter(lambda g: len(g) >= 20)
...
seq_agg_df.to_csv('../outputs/gene_ontology/SeqFish_peripheral.csv',index=False)
