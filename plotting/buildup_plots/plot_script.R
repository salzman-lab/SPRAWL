...
if(nrow(srrs_df) == 0){
    ...
    stop()
} else if (nrow(srrs_df) <= 4){
    #plot all cell types
    num_ontologies = nrow(srrs_df)
    cell_types = srrs_df$ontology
    effect_sizes = srrs_df$med_score
} else {
    #plot top 2 and bottom 2 cell-types by med score (effect size)
    num_ontologies = 4
    cell_types = c(head(srrs_df,2)$ontology, tail(srrs_df,2)$ontology)
    effect_sizes = c(head(srrs_df,2)$med_score, tail(srrs_df,2)$med_score)
}
