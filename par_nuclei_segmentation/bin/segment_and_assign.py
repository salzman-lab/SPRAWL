...
def main():
    ...
    fov_micron_min_x,fov_micron_min_y,fov_micron_max_x,fov_micron_max_y = get_fov_boundaries(args.hdf5_fov)
    ...
    #Read in part of the DAPI stain image
    im = tifffile.memmap(args.mosaic)
    ...
    fov_gdf = geopandas.GeoDataFrame({
        'cell_id':cell_ids,
        'geometry':micron_nuclei_polys,
    })

    #error thrown when trying to write an empty geopandas df (seems like a bug)
    if fov_gdf.empty:
        with open('cell_nuc.gpkg','w') as f:
            f.write('empty file\n')
    else:
        fov_gdf['area'] = fov_gdf.area
        fov_gdf['sample'] = args.sample
        fov_gdf['z'] = args.z
        fov_gdf['fov'] = args.fov
        fov_gdf.to_file('cell_nuc.gpkg', driver='GPKG')
